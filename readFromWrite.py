...
"""
import os, re, glob, nuke
FILEPATH_KNOBS = [
 'file']
SINGLE_FILE_FORMATS = [
 'avi', 'mp4', 'mxf', 'mov', 'mpg', 'mpeg', 'wmv', 'm4v',
 'm2v']
...
class ReadFromWrite(object):
    ...
    def combined_relative_filepath_exists(self, relative_filepath, return_filepath=False):
        ...
        frame_number = re.findall('\\d+', filepath)[(-1)]
    ...
    def filepath_from_disk(self, node, knob_value, knob_eval):
        ...
        if os.path.exists(knob_value):
            ...
            filepath = knob_eval
        elif not isinstance(self.project_dir(), type(None)) and self.combined_relative_filepath_exists(knob_eval):
            filepath = self.combined_relative_filepath_exists(knob_eval, return_filepath=True)
        return filepath
    ...
    def process_filepath(self, filepath, filetype, basename, padding, knob_value):
        ...
        filepath_process = self.determine_image_type(filepath_process, basename, padding, filetype)
    ...
    def node_options(self, node):
        ...
        options = {'colorspace': colorspace, 'premultiplied': premultiplied, 
           'raw': raw}
        return options
    ...
    def frame_info(self, node, knob_value, knob_eval):
        ...
        if isinstance(filepath, type(None)):
            not_found = 'Filepath does not exist and/or cannot be' + 'translated:\n' + node + ': ' + knob_eval + '\n\nCreate Read node anyway and guess framerange?'
            if nuke.ask(not_found):
                limit_to_range = self.get_knob_value(node, 'use_limit')
                if not isinstance(limit_to_range, type(None)) and int(limit_to_range) == 1:
                    firstframe = int(self.get_knob_value(node, 'first'))
                    lastframe = int(self.get_knob_value(node, 'last'))
                else:
                ...
                frame_data = {'filepath': filepath_processed, 
            ...
            filepath = filepath.replace('\\', '/')
            current_frame = re.findall('\\d+', filepath)[(-1)]
            ...
            filepath_processed = self.process_filepath(filepath, filetype, basename, padding, knob_value)
            node_options = self.node_options(node)
            frame_data = {'filepath': filepath_processed, 
               'firstframe': firstframe, 
               'lastframe': lastframe, 
            ...
            for knob in data[node]:
                if isinstance(data[node][knob], type(None)):
                    ...
                    for node_option in node_options:
                        self.set_knob_from_data(node, data, knob, r, data_key=node_option, is_option=True)
